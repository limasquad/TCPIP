...
int main(int argumentCount, char* argumentVector[])
{
	...
	//fileAddress0 = fopen("ReceivedText.txt", "r+b"); //read+ binary
	fileAddress0 = fopen("ReceivedText.txt", "wt+"); //write+ text
	//fileAddress0 = fopen("ReceivedText.txt", "at+"); //append+
	...
	while (1)
	{
		...
		{
			int receivedSize0 = ReceiveDatum(clientSocket[0], receiveBuffer, BUFFER_SIZE, 0);
			...
			fputs(receiveBuffer, fileAddress0);
